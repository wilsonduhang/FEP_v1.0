...
## 当前阶段
S4 数据脱敏统一（DesensitizeService）— 详见 `../FEP/docs/plans/PHASE_HISTORY.md`
（DesensitizeService[security-api] + DesensitizeServiceImpl[无 stereotype，DesensitizeConfiguration @Bean always-on] 按架构 §680-682 脱敏身份证/银行卡/手机号/USCI + @Desensitize 注解 + DesensitizeJsonSerializer[ContextualSerializer] + WebJacksonConfiguration[SpringHandlerInstantiator] DTO 层声明式脱敏 + UserResponse phone 整合。前序 S1 国密 SM4 + KeyServiceImpl + 惰性迁移已 MERGED。完整状态见 `../FEP/CLAUDE.md` 当前项目状态段）
