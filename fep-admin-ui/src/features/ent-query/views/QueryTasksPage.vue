  ...
  <div class="query-tasks-page">
    ...
    >
      <template #queryType="{ row }">
        <StatusTag
          :value="row.queryType"
          :mapping="QUERY_TYPE_MAP"
        />
      </template>
      <template #taskStatus="{ row }">
        <StatusTag
          :value="row.taskStatus"
          :mapping="QUERY_TASK_STATUS_MAP"
        />
      </template>
      <template #operation>
        <el-table-column
    ...
    <QueryTaskCreateDialog
      v-model="dialogVisible"
      @created="refresh"
    />
    <QueryTaskDetailDrawer
...
const page = ref<PageResult<QueryTaskResponse>>({ records: [], total: 0, pageNum: 1, pageSize: 20, totalPages: 0 });
...
function onReset() {
  Object.assign(searchForm, { pageNum: 1, pageSize: 20, queryType: undefined, taskStatus: undefined, keyword: undefined });
}
...
<style scoped>
.query-tasks-page { display: flex; flex-direction: column; gap: 16px; }
