...
/**
 * 3108 平台凭证核对回执报文业务体 (PRD §4.4)。
 *
 * <p>字段顺序严格对应 {@code 3108.xsd} 中 {@code pzCheckQueryReturn3108} complexType 的
 * sequence (7 字段): SerialNo, SendNodeCode, DesNodeCode, CheckDate, hxqyNum,
 * pzCheckReturn[0..200]?, ExtInfo?。前 5 字段 required；pzCheckReturn 列表与 ExtInfo 均
 * optional ({@code minOccurs="0"})。</p>
 *
 * <p><b>嵌套类型</b>：
 * <ul>
 *   <li>{@code List<}{@link PzCheckReturn}{@code >} — 平台凭证核对回执列表
 *       ({@code minOccurs="0"} / {@code maxOccurs="200"})，supplychain 局部支撑类型</li>
...
@XmlRootElement(name = "pzCheckQueryReturn3108")
@XmlType(name = "pzCheckQueryReturn3108", propOrder = {
        "serialNo", "sendNodeCode", "desNodeCode",
        "checkDate", "hxqyNum",
        "pzCheckReturn",
        "extInfo"
})
public class PzCheckQueryReturn3108 extends CfxBody implements SerialNoBearing {
    ...
    @XmlElement(name = "pzCheckReturn")
    private List<PzCheckReturn> pzCheckReturn;
    ...
    /**
     * Returns the optional voucher reconciliation result list
     * (平台凭证核对回执列表, optional, 0-200 entries).
     *
     * @return voucher reconciliation result list, or {@code null} if absent
     */
    public List<PzCheckReturn> getPzCheckReturn() {
        return pzCheckReturn;
    }

    /**
     * Sets the optional voucher reconciliation result list.
     *
     * @param v voucher reconciliation result list
     */
    public void setPzCheckReturn(final List<PzCheckReturn> v) {
        this.pzCheckReturn = v;
    }
