...
public class TlqInboundListener implements MessageListener {
    ...
    /**
     * HNDEMP transitionNo length (PRD §3.2.4 — last 8 digits of msgId).
     */
    ...
    public void onMessage(final TlqMessage message) {
        ...
        try {
            ...
            final String transitionNo = deriveTransitionNo(head.getMsgId());
    ...
    /**
     * Derive a transitionNo from the CFX {@code msgId} by taking its last
     * {@value #TRANSITION_NO_LEN} characters.
     *
     * <p>Aligns with the ADR-P2e-2 convention that transitionNo is a
     * pseudo-derived 8-digit suffix of the 20-digit msgId until the
     * dispatcher receives an explicit RealHead.transitionNo via the inbound
     * Body POJO (see ADR-P2e-2 / Plan v1a §4 Phase 2 scope).</p>
     *
