...
/**
 * {@link InAppNotificationRepository} 行为验证。
 *
...
class InAppNotificationRepositoryTest {
    ...
    void findUnread_shouldReturnOnlyUnreadOrderedByCreateTimeDesc() {
        repository.save(InAppNotificationEntity.of("admin-1", "CALLBACK_DLQ", "ERROR",
                "t1", "m1", "Q1", "CALLBACK_DLQ_ENTRY"));
        repository.save(InAppNotificationEntity.of("admin-1", "CALLBACK_DLQ", "ERROR",
                "t2", "m2", "Q2", "CALLBACK_DLQ_ENTRY"));
        final InAppNotificationEntity readOne = InAppNotificationEntity.of("admin-1",
                "CALLBACK_DLQ", "ERROR", "t3", "m3", "Q3", "CALLBACK_DLQ_ENTRY");
        readOne.markRead();
        repository.save(readOne);

        final List<InAppNotificationEntity> unread =
                repository.findByUserIdAndReadFalseOrderByCreateTimeDesc("admin-1");

        assertThat(unread).hasSize(2)
                .extracting(InAppNotificationEntity::getTitle)
                .containsExactlyInAnyOrder("t1", "t2");
    }

    @Test
    void countUnread_shouldExcludeReadAndOtherUsers() {
        repository.save(InAppNotificationEntity.of("admin-2", "CALLBACK_DLQ", "ERROR",
                "t1", "m1", "Q1", "CALLBACK_DLQ_ENTRY"));
        final InAppNotificationEntity readOne = InAppNotificationEntity.of("admin-2",
                "CALLBACK_DLQ", "ERROR", "t2", "m2", "Q2", "CALLBACK_DLQ_ENTRY");
        readOne.markRead();
        repository.save(readOne);
        repository.save(InAppNotificationEntity.of("other-user", "CALLBACK_DLQ", "ERROR",
    ...
    void markRead_shouldFlipFlagAndStampReadAt() {
        final InAppNotificationEntity e = repository.saveAndFlush(InAppNotificationEntity.of(
                "admin-3", "CALLBACK_DLQ", "ERROR", "t1", "m1", "Q1", "CALLBACK_DLQ_ENTRY"));
        e.markRead();
        repository.saveAndFlush(e);

        final InAppNotificationEntity found = repository.findById(e.getNotificationId()).orElseThrow();
