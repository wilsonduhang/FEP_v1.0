...
class SysDataTypeConfigControllerTest {
    ...
    /**
     * 创建有效数据类型应返回 200 并持久化数据。
     *
     * @throws Exception MockMvc 请求异常
     */
    @Test
    void create_shouldPersist() throws Exception {
        DataTypeConfigCreateRequest request = new DataTypeConfigCreateRequest();
        request.setTypeName("字符串类型");
        request.setTypeCode("DT_STRING_TEST");

        mockMvc.perform(post("/api/v1/sys/config/data-types")
                        .header("Authorization", "Bearer " + accessToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.dataTypeId", notNullValue()))
                .andExpect(jsonPath("$.data.typeName", is("字符串类型")))
                .andExpect(jsonPath("$.data.typeCode", is("DT_STRING_TEST")))
                .andExpect(jsonPath("$.data.typeStatus", is("ENABLED")));
    }

    /**
     * 创建重复编码的数据类型应返回 409 Conflict。
     *
     * @throws Exception MockMvc 请求异常
     */
    @Test
    void create_duplicateCode_shouldReturn409() throws Exception {
        DataTypeConfigCreateRequest req = new DataTypeConfigCreateRequest();
        req.setTypeName("整数类型");
        req.setTypeCode("DT_INT_DUP_TEST");
    ...
    void search_shouldReturnPagedResults() throws Exception {
        mockMvc.perform(get("/api/v1/sys/config/data-types")
                        .header("Authorization", "Bearer " + accessToken)
                        .param("pageNum", "1")
                        .param("pageSize", "10"))
                .andExpect(status().isOk())
    ...
    void update_shouldModify() throws Exception {
        // 先创建
        DataTypeConfigCreateRequest createReq = new DataTypeConfigCreateRequest();
        createReq.setTypeName("原数据类型");
        createReq.setTypeCode("DT_UPDATE_TEST");

        MvcResult createResult = mockMvc.perform(post("/api/v1/sys/config/data-types")
                        .header("Authorization", "Bearer " + accessToken)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        String dataTypeId = objectMapper.readTree(createResult.getResponse().getContentAsString())
                .at("/data/dataTypeId").asText();

        // 更新
        DataTypeConfigCreateRequest updateReq = new DataTypeConfigCreateRequest();
        updateReq.setTypeName("新数据类型");
        updateReq.setTypeCode("DT_UPDATE_TEST");

        mockMvc.perform(put("/api/v1/sys/config/data-types/" + dataTypeId)
                        .header("Authorization", "Bearer " + accessToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(updateReq)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.typeName", is("新数据类型")));
    }
    ...
    void delete_shouldRemove() throws Exception {
        // 先创建
        DataTypeConfigCreateRequest createReq = new DataTypeConfigCreateRequest();
        createReq.setTypeName("待删除数据类型");
        createReq.setTypeCode("DT_DELETE_TEST");
        ...
        // 删除
        mockMvc.perform(delete("/api/v1/sys/config/data-types/" + dataTypeId)
                        .header("Authorization", "Bearer " + accessToken))
                .andExpect(status().isOk());
    }
}
