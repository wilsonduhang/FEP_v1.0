 ...
 * 现状（2026-06-01，P4-MSG-L T2 同步）注册 23 个：
 ...
 * 9007/9009（P4-MSG-L 节点登录/登出回执，无业务 SerialNo 字段，实现返回 null）。</p>
 *
...
class InboundRegistryArchTest {
    ...
    @Test
    void registry_currentSnapshot_hasTwentyThreeEntries() {
        // Snapshot guard — 提示开发者注册新 body 时同步更新 SerialNoBearingComplianceTest.
        // 23 项 = 1 P4-MSG-D（2101，null fallback）+ 3 P4-MSG-A-inbound BATCH（2102/2103/2104,
        ...
        Map<String, Class<?>> registry = InboundMessageDispatcher.bodyTypeRegistry();
        assertThat(registry).hasSize(23);
